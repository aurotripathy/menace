...
class GroupedBlock(nn.Module):
    expansion = 2
    def __init__(self, in_planes, cardinality=32, bottleneck_width=4,
                 stride=1):
...
class ResNeXt(nn.Module):
    def __init__(self, num_blocks_list, cardinality, bottleneck_width,
                 num_classes=10):
        ...
        self.layer1 = self._make_layer(num_blocks_list[0], 1)
        self.layer2 = self._make_layer(num_blocks_list[1], 2)
        self.layer3 = self._make_layer(num_blocks_list[2], 2)
        # self.layer4 = _make_layer(num_blocks_list[3], 2)
    ...
    def _make_layer(self, num_blocks, stride):
        ...
        for stride in strides:
            layers.append(GroupedBlock(self.in_planes, self.cardinality,
                                           self.bottleneck_width, stride))
            self.in_planes = GroupedBlock.expansion * self.cardinality * self.bottleneck_width
