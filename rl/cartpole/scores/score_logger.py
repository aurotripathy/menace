...
class ScoreLogger:
    ...
    def add_score(self, score, run):
        ...
        print("Scores: (min: " + str(min(self.scores)) + ", avg: " +
        ...
        if mean_score >= AVERAGE_SCORE_TO_SOLVE and len(self.scores) >= CONSECUTIVE_RUNS_TO_SOLVE:
            ...
            print("Solved in " + str(solve_score) +
                  " runs, " + str(run) + " total runs.")
    ...
    def _save_png(self, input_path, output_path, x_label, y_label, average_of_n_last, show_goal, show_trend, show_legend):
        ...
        average_range = average_of_n_last if average_of_n_last is not None else len(
            x)
        plt.plot(x[-average_range:], [np.mean(y[-average_range:])] * len(y[-average_range:]),
                 linestyle="--", label="last " + str(average_range) + " runs average")
